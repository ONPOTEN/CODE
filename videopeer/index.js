...
// CRITICAL: Pass Express app to http.createServer so API routes work
...
app.get("/", (req, res) => {
    res.send("VideoPeer Socket.IO Server is running");
});

// Test endpoint to verify HTTP server is working
app.get("/test", (req, res) => {
    console.log('[TEST] Test endpoint called:', req.url);
    res.json({ message: 'HTTP server is working', timestamp: new Date().toISOString() });
});
...
// Notification emission endpoint - receives from Laravel and broadcasts via Socket.IO
app.post("/api/emit-notification", (req, res) => {
    const { recipientId, notificationData } = req.body;

    console.log(`\n🔔 [NOTIFICATION DEBUG] Received from Laravel`);
    console.log(`   Timestamp: ${new Date().toISOString()}`);
    console.log(`   Recipient ID (postOwnerId): ${recipientId}`);
    console.log(`   Notification Type: ${notificationData.type}`);
    console.log(`   Post Type: ${notificationData.posttype}`);
    console.log(`   Post ID: ${notificationData.postid}`);
    console.log(`   Sender ID (userid): ${notificationData.userid}`);
    console.log(`   Full notificationData:`, JSON.stringify(notificationData, null, 2));

    if (!recipientId || !notificationData) {
        console.error('❌ Invalid emit-notification request:', req.body);
        return res.status(400).json({ error: 'Missing recipientId or notificationData' });
    }

    // DEBUG: Log all registered user mappings
    console.log(`\n📋 [DEBUG] Current userIdToSocket mappings:`);
    for (const [userId, socketId] of userIdToSocket.entries()) {
        const socket = io.sockets.sockets.get(socketId);
        console.log(`   User ID ${userId} -> Socket ${socketId} (connected: ${socket ? 'YES' : 'NO'})`);
    }

    // Emit to recipient's socket if connected
    const recipientSocketId = userIdToSocket.get(recipientId.toString());

    if (recipientSocketId) {
        const socket = io.sockets.sockets.get(recipientSocketId);
        console.log(`\n✅ [DEBUG] Found recipient socket mapping:`);
        console.log(`   User ID: ${recipientId}`);
        console.log(`   Socket ID: ${recipientSocketId}`);
        console.log(`   Socket exists: ${socket ? 'YES' : 'NO'}`);

        // Emit the correct event name that the client is listening for
        const eventName = `engagement:${notificationData.type}-added`;
        console.log(`   Emitting event: ${eventName}`);

        // Create the payload format expected by the client
        const payload = {
            post_id: notificationData.postid,
            post_type: notificationData.posttype,
            comment: notificationData.type === 'comment' ? {
                id: notificationData.id,
                post_id: notificationData.postid,
                content: notificationData.content,
                author: {
                    id: notificationData.userid,
                    name: 'Commenter', // Could be enhanced to fetch actual name
                }
            } : null,
            like: notificationData.type === 'like' ? {
                id: notificationData.id,
        ...
        console.log(`✅ Notification sent to user ${recipientId} (socket: ${recipientSocketId})`);

        // Also broadcast the old way for compatibility
        io.to(recipientSocketId).emit('new:notification', notificationData);
        console.log(`   Also sent as 'new:notification' for compatibility`);
    } else {
        console.log(`\n⚠️  [DEBUG] Recipient ${recipientId} not found in userIdToSocket map`);
        console.log(`   Available user IDs: ${Array.from(userIdToSocket.keys()).join(', ')}`);

        // Fall back to broadcasting to all sockets
        const eventName = `engagement:${notificationData.type}-added`;
        const payload = {
            post_id: notificationData.postid,
            post_type: notificationData.posttype,
        ...
        console.log(`📢 [FALLBACK] Broadcasted to all connected sockets as ${eventName}`);
    }

    // Also broadcast to all (frontend will filter by recipient) - old event name for compatibility
    io.emit('new:notification', notificationData);
    console.log(`📢 Also broadcasted 'new:notification' to all connected sockets`);
