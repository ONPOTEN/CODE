...
export default function UserWallPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMessage('Đã tạo bài viết thành công trên tường của bạn!');
      setFormData({
        title: '',
        content: '',
        excerpt: '',
  ...
  useEffect(() => {
    async function fetchWallPosts() {
      ...
      try {
        ...
        if (err instanceof ApiException) {
          setError(`Không thể tải bài viết trên tường: ${err.message}`);
        } else {
          setError('Không thể tải bài viết trên tường');
        }
  ...
  const handleDeleteSharedPost = async (postId: number) => {
    ...
    if (!confirm('Bạn có chắc chắn muốn xóa bài viết đã chia sẻ này khỏi tường của mình?')) {
      return;
    }
    ...
    try {
      ...
      alert('Đã xóa bài viết đã chia sẻ khỏi tường của bạn!');

      console.log('[UserWallPage] Shared post deleted successfully');
    } catch (err) {
      if (err instanceof ApiException) {
        alert(`Không thể xóa bài viết đã chia sẻ: ${err.message}`);
      } else {
        alert('Không thể xóa bài viết đã chia sẻ');
      }
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-grey-200 rounded-lg shadow-sm p-6 mb-6">
          ...
          {user.hobby && <p className="text-sm text-gray-600">Sở thích: {user.hobby}</p>}
          {user.company && <p className="text-sm text-gray-600">Công ty: {user.company}</p>}
          {user.location && <p className="text-sm text-gray-600">Địa điểm: {user.location}</p>}
        </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">
                  {selectedImages.length}/10 hình ảnh đã chọn
                </p>
              </div>
        ...
        {wallLoading && sharedWallLoading ? (
          ...
          <div className="bg-grey-200 rounded-lg shadow-sm p-8 text-center">
            <p className="text-gray-600">Không tìm thấy bài viết nào trên tường này.</p>
          </div>
        ) : (
          <>
            ...
            {wallPosts.length > 0 && (
              <div className="mb-8">
                <div className="flex items-center gap-2 mb-4">
                  <h2 className="text-xl font-bold text-gray-900">Bài viết trên tường</h2>
            ...
            {sharedWallPosts.length > 0 && (
              <div className="mb-8">
                <div className="flex items-center gap-2 mb-4">
                  <h2 className="text-xl font-bold text-gray-900">Bài viết được chia sẻ</h2>
                  <span className="text-sm text-gray-500 bg-blue-500 px-3 py-1 rounded-full">
                                    ...
                                    {openSharedPostMenuId === post.id && (
                                      <div className="absolute right-0 mt-2 w-48 bg-grey-200 rounded-lg shadow-lg border border-gray-300 z-50 py-1">
                                        ...
                                        >
                                          ...
                                          {deletingSharedPostId === post.id ? 'Đang xóa...' : 'Xóa khỏi tường'}
                                        </button>
                                      </div>
                                    )}
                                  </div>
            ...
            {wallPosts.length === 0 && sharedWallPosts.length > 0 && (
              <div className="bg-white rounded-lg shadow-sm p-8 text-center">
                <p className="text-gray-600">Không có bài viết nào trên tường này, nhưng có các bài viết được chia sẻ bên dưới.</p>
              </div>
            )}
          </>
        )}
