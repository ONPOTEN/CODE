...
export default function MessagesPage() {
  ...
  const handleNewMessageCallback = useCallback((message?: any) => {
    ...
    setConversations(prevConversations => {
      const conversationIndex = prevConversations.findIndex(c => c.id === selectedConversationId);
      if (conversationIndex === -1) return prevConversations;
  ...
  return (
    <div className={`flex flex-col h-screen bg-[#F8FAFC] transition-[margin-left] duration-300 ease-in-out ${isSidebarVisible ? 'lg:ml-sidebar' : ''}`}>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-full lg:w-80 xl:w-[400px] flex flex-col border-r border-slate-100 bg-white shadow-xl shadow-slate-200/50 relative z-30">
          {/* List Header */}
          ...
          {/* Search/Filter Bar (Optional visual polish) */}
          <div className="px-5 py-3">
            <div className="relative">
              <input 
                type="text" 
                placeholder="Search conversations..." 
                className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border-none rounded-2xl text-xs font-bold text-slate-600 placeholder-slate-400 focus:ring-2 focus:ring-indigo-600/10 transition-all"
              />
        ...
        <div className="hidden lg:flex lg:flex-1 flex-col overflow-hidden bg-slate-50">
          {selectedConversation ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-12 text-center bg-[#F8FAFC]">
              <div className="relative mb-8">
                ...
                <div className="relative w-32 h-32 rounded-[3rem] bg-white shadow-2xl flex items-center justify-center text-6xl">
                  👋
                </div>
                <div className="absolute -bottom-2 -right-2 w-10 h-10 rounded-2xl bg-indigo-600 text-white flex items-center justify-center shadow-lg shadow-indigo-200">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="max-w-xs space-y-3">
                ...
                <p className="text-slate-400 font-bold text-sm leading-relaxed uppercase tracking-widest">
                  Pick a conversation from the sidebar to continue your journey
                </p>
              </div>
            </div>
          )}
