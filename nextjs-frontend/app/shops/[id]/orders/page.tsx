  ...
  useEffect(() => {
    async function fetchShopAndOrders() {
      try {
        ...
        // Ghi lại phản hồi thô để gỡ lỗi
        console.log('[ShopOrders] Phản hồi cửa hàng thô:', shopResponse);

        // Xử lý cả đối tượng trực tiếp và phản hồi được gói gọn
        let shopInfo = (shopResponse as any)?.data || shopResponse;

        // Đôi khi dữ liệu cửa hàng có thể lồng sâu hơn
        if (shopInfo?.shop) {
          shopInfo = shopInfo.shop;
        }

        console.log('[ShopOrders] Thông tin cửa hàng đã xử lý:', {
          id: shopInfo?.id,
          name: shopInfo?.name,
        ...
        // Kiểm tra xem người dùng có sở hữu cửa hàng này không - chủ cửa hàng phải có user_id khớp
        // Chuyển đổi cả hai thành số để so sánh để xử lý sự không khớp kiểu chuỗi/số
        const shopOwnerId = parseInt(String(shopInfo?.user_id || shopInfo?.owner?.id || '0'), 10);
        const userId = parseInt(String(user?.id || '0'), 10);

        console.log('[ShopOrders] Kiểm tra quyền:', {
          shopOwnerId,
          userId,
        ...
        if (!shopOwnerId || shopOwnerId <= 0 || shopOwnerId !== userId) {
          console.warn('[ShopOrders] Quyền bị từ chối - chủ sở hữu cửa hàng không khớp', {
        ...
        let errorMsg = 'Không thể lấy đơn hàng cửa hàng';
