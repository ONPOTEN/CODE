...
export default function EditGroupPostPage() {
  ...
  useEffect(() => {
    async function fetchPost() {
      try {
        setIsFetching(true);
        const response = await groupPosts.getById(postId);
        ...
        setFormData({
          ...
          status: postData.post_status || 'draft',
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      formDataToSend.append('_method', 'POST'); // Laravel requires this for form-data updates
      ...
      // Set remove_video flag if user wants to remove existing video without replacing
      if (removeExistingVideo && !selectedVideo) {
        formDataToSend.append('remove_video', 'true');
      }

      await groupPosts.updateWithFiles(postId, formDataToSend);
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="max-w-3xl mx-auto">
        <h1 className="text-3xl font-bold mb-8">Sửa bài viết nhóm</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="mt-2">
                <label className="flex items-center">
                  <input
                    ...
                    className="mr-2"
                  />
                  <span className="text-sm text-gray-600">Thay thế tất cả ảnh cũ khi tải ảnh mới</span>
                </label>
              </div>
            </div>
          )}
          ...
          <div className="flex gap-4">
            <button
              type="submit"
              disabled={isLoading}
              className="bg-blue-500 hover:bg-blue-700 text-white font-medium py-2 px-6 rounded transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
