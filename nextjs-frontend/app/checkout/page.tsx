...
export default function CheckoutPage() {
  ...
  const [orderReference] = useState(() => {
    // Tạo tham chiếu đơn hàng duy nhất để theo dõi thanh toán
  ...
  // Ánh xạ mã ngân hàng cho VietQR
  ...
  // Tải mã QR cài đặt thanh toán cửa hàng
  useEffect(() => {
    const loadShopPaymentSettings = async () => {
      try {
        // Lấy ID cửa hàng từ mục đầu tiên trong giỏ hàng
        if (!items || items.length === 0) return;

        // Trích xuất shop_id từ mục đầu tiên (giả sử tất cả các mục từ cùng một cửa hàng)
        const firstItem = items[0];
        const shopId = firstItem.shopId;

        if (!shopId) {
          console.log('Không tìm thấy shopId trong mục giỏ hàng');
          return;
        }

        setLoadingQR(true);
        try {
          // Tải cài đặt thanh toán cho cửa hàng này
          const paymentSettingsResponse = await apiRequest(`/shops/${shopId}/payment-settings`);
          const settings = paymentSettingsResponse?.data;

          // Lưu thông tin thanh toán cửa hàng để hiển thị
          ...
          if (!settings?.qr_code) {
            ...
            if (bankCode) {
              ...
              // Định dạng: "DH123ABC TT 790000d" - Tham chiếu đơn hàng + Tổng giá
          ...
          console.log('Không thể tải mã QR cho cửa hàng này:', err);
          // Điều này không quan trọng - mã QR là tùy chọn
        } finally {
          setLoadingQR(false);
        }
      } catch (err) {
        console.error('Lỗi khi tải cài đặt thanh toán cửa hàng:', err);
      }
    };
  ...
  const validateForm = () => {
    ...
    // Xác thực địa chỉ giao hàng cho tất cả các phương thức thanh toán
  ...
  const handlePlaceOrder = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('Đã tạo đơn hàng thành công:', response);

      // Hiển thị thành công và xóa giỏ hàng
      setOrderPlaced(true);
      clearCart();

      // Chuyển hướng đến trang my-orders sau 3 giây
      setTimeout(() => {
      ...
      if (error instanceof ApiException) {
        ...
        // Ghi nhật ý các lỗi xác thực để gỡ lỗi
        if (error.errors) {
          console.error('Lỗi xác thực:', error.errors);
          // Hiển thị lỗi xác thực đầu tiên nếu có
