...
export function ShareToWallModal({
  ...
  const handleShare = async () => {
    if (!selectedPostId) {
      setError('Vui lòng chọn một bài viết');
      return;
    }
    ...
    try {
      ...
      alert(`Đã chia sẻ bài viết lên tường của ${wallUserName} thành công!`);
      ...
      if (err instanceof ApiException) {
        ...
        setError('Không thể chia sẻ bài viết lên tường');
      }
      ...
      <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-full max-w-lg mx-4 bg-gray-50 rounded-lg shadow-xl z-50 max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-gray-50 border-b border-gray-300 p-6 flex items-center justify-between">
          <h2 className="text-2xl font-bold text-gray-900">
            Đăng lên tường của {wallUserName}
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="p-6">
          ...
          {loading ? (
            ...
            <div className="text-center py-8 text-gray-500">
              ...
              <p className="text-sm">Bạn chưa tạo bài viết đã xuất bản nào</p>
            </div>
          ) : filteredPosts.length === 0 ? (
            <div className="text-center py-8 text-gray-500">
              <p className="text-sm">Không có bài viết nào khớp với tìm kiếm của bạn</p>
            </div>
          ) : (
