...
export default function ShopMessageModal({
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    if (!user) {
      setError('Bạn phải đăng nhập để gửi tin nhắn');
      return;
    }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-50 rounded-lg shadow-xl max-w-md w-full">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-300">
          <div>
            <h2 className="text-xl font-bold text-gray-900">Nhắn tin cho cửa hàng</h2>
        ...
        <form onSubmit={handleSendMessage} className="p-6 space-y-4">
          ...
          {success && (
            <div className="p-3 bg-green-50 border border-green-200 rounded-lg flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-medium text-green-800">Đã gửi tin nhắn thành công!</p>
                <p className="text-xs text-green-700">Cửa hàng sẽ nhận được tin nhắn của bạn sớm.</p>
              </div>
            </div>
          )}
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Nhập tin nhắn của bạn ở đây... (ví dụ: câu hỏi, thắc mắc về sản phẩm, phản hồi)"
        ...
        <div className="px-6 py-3 bg-blue-50 border-t border-gray-300 rounded-b-lg">
          <p className="text-xs text-blue-700">
            Tin nhắn của bạn sẽ được gửi đến cửa hàng thông qua Socket.IO nhắn tin thời gian thực.
          </p>
        </div>
      </div>
    </div>
