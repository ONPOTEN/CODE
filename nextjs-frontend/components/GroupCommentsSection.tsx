      ...
      <div className="flex-shrink-0">
        {(comment.author?.avatar || (comment as any).user?.avatar || (comment as any).user?.avatar_url) ? (
          <img
            ...
            alt={comment.author?.name || (comment as any).user?.name || comment.author_name || 'Người dùng'}
            className="w-8 h-8 rounded-full object-cover"
          />
        ) : (
          <div className="w-8 h-8 bg-blue-400 rounded-full flex items-center justify-center text-gray-900 text-sm font-bold">
            {(comment.author?.name || (comment as any).user?.name || comment.author_name || 'N').charAt(0).toUpperCase()}
          </div>
        )}
      </div>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2">
          {(comment.author?.id || (comment as any).user?.id || comment.user_id) ? (
            ...
            >
              {comment.author?.name || (comment as any).user?.name || comment.author_name || 'Ẩn danh'}
            </Link>
          ) : (
