    ...
    <div className="bg-gray-50 rounded-lg shadow-md p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1">
            Tin nhắn từ khách hàng quan tâm đến cửa hàng của bạn
          </p>
        </div>
        {messages.length > 0 && (
          <div className="bg-blue-100 text-blue-800 rounded-full px-3 py-1 font-semibold text-sm">
            {messages.length} {messages.length === 1 ? 'tin nhắn' : 'tin nhắn'}
          </div>
        )}
      </div>
      ...
      <div className="mb-4 space-y-2">
        ...
        {!isConnected && (
          <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-lg flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-yellow-800">Chưa kết nối Socket.IO</p>
              <p className="text-xs text-yellow-700">Tin nhắn thời gian thực sẽ không được nhận cho đến khi kết nối được khôi phục</p>
            </div>
          </div>
        )}
      </div>
      ...
      {messages.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-gray-500 font-medium mb-1">Chưa có tin nhắn nào</p>
          <p className="text-gray-600 text-sm">Tin nhắn từ khách hàng sẽ hiển thị ở đây</p>
        </div>
      ) : (
      ...
      {messages.length > 0 && (
        <div className="mt-6 pt-4 border-t border-gray-300">
          <p className="text-xs text-gray-600 text-center">
            Tin nhắn được lưu theo thời gian thực. Làm mới để xem tất cả tin nhắn được gửi khi bạn vắng mặt.
          </p>
        </div>
      )}
    </div>
