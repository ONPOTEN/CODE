...
export default function GroupPostCard({
  ...
  const handleApprovePost = async () => {
    ...
    try {
      ...
      setModerationError(err.message || 'Không thể duyệt bài viết');
    } finally {
  ...
  const handleRejectPost = async () => {
    ...
    try {
      ...
      setModerationError(err.message || 'Không thể từ chối bài viết');
    } finally {
          ...
          {post.post_status && (
            <div>
              {post.post_status === 'pending' && (
                <span className="inline-block px-2 py-1 bg-yellow-500 text-gray-900 text-xs font-bold rounded-full">⏳ Chờ duyệt</span>
              )}
              {post.post_status === 'trash' && (
                <span className="inline-block px-2 py-1 bg-red-500 text-gray-900 text-xs font-bold rounded-full">🚫 Đã từ chối</span>
              )}
              {post.post_status === 'publish' && (
                <span className="inline-block px-2 py-1 bg-green-500 text-gray-900 text-xs font-bold rounded-full">✓ Đã xuất bản</span>
              )}
            </div>
          )}
      ...
      <div className="p-6">
        ...
        <div className="flex gap-6 pt-4 border-t border-gray-300 text-sm text-gray-600">
          ...
          >
            ...
            <span>{dislikes} Không thích</span>
          </button>
        ...
        {(post.post_status === 'pending' || post.post_status === 'trash') && (
          <div className={`mt-4 px-3 py-2 rounded-lg text-sm font-medium ${post.post_status === 'pending' ? 'bg-yellow-50 text-yellow-700 border border-yellow-200' : 'bg-red-50 text-red-700 border border-red-200'}`}>
            {post.post_status === 'pending' ? '⏳ Chờ duyệt' : '🚫 Đã từ chối'}
          </div>
        )}
        ...
        {isUserModerator && post.post_status === 'pending' && (
          <div className="mt-4 flex gap-3">
            ...
            >
              {isModeratingApprove ? 'Đang duyệt...' : '✓ Duyệt bài viết'}
            </button>
            <button
            ...
            >
              {isModeratingReject ? 'Đang từ chối...' : '✗ Từ chối bài viết'}
            </button>
          </div>
        )}
