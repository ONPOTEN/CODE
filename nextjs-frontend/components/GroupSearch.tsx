  ...
  const handleSearch = useCallback(
    async (term: string) => {
      ...
      try {
        ...
        setError('Không thể tìm kiếm nhóm');
      } finally {
        setLoading(false);
      }
    },
    ...
    <section className="container py-12 bg-gray-50">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-8">
          <h2 className="text-3xl font-bold text-gray-900 mb-2">Khám phá Nhóm</h2>
          <p className="text-gray-600">Tìm và tham gia cộng đồng phù hợp với sở thích của bạn</p>
        </div>
          ...
          {hasSearched && searchTerm && (
            <div className="mt-3 text-sm text-gray-600">
              {loading ? (
                ...
                <span>Không tìm thấy nhóm nào khớp với "{searchTerm}"</span>
              ) : (
                <span>Tìm thấy {results.length} nhóm</span>
              )}
            </div>
          )}
        </div>
          ...
          <div>
            {loading ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="text-gray-500 mb-4">Không tìm thấy nhóm nào khớp với tìm kiếm của bạn</p>
                <Link
                  href="/groups"
        ...
        {!hasSearched && (
          <div className="rounded-lg p-8 text-center border border-blue-100">
            <p className="text-gray-600 mb-4">
              Tìm kiếm nhóm ở trên hoặc khám phá tất cả cộng đồng có sẵn
            </p>
            <Link
              href="/groups"
              className="inline-block px-6 py-2 bg-blue-600 text-gray-900 rounded-lg hover:bg-blue-700 font-medium transition-colors"
