...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const syncWithNextAuth = async (token: string, userData: any) => {
    try {
      ...
      console.error('Failed to sync auth with NextAuth', e);
    }
  };
  ...
  useEffect(() => {
    ...
    if ((session as any)?.apiToken && session?.user) {
      const apiToken = (session as any).apiToken as string;
      tokenStorage.set(apiToken);
      setUser(session.user as any);
      setIsLoading(false);

      // Emit token update event for EngagementProviderWrapper to catch
      if (typeof window !== 'undefined') {
        window.dispatchEvent(new CustomEvent('tokenUpdated', { detail: apiToken }));
      }
    } else {
      ...
      if (localToken && typeof window !== 'undefined') {
        window.dispatchEvent(new CustomEvent('tokenUpdated', { detail: localToken }));
      }
    }
    ...
    try {
      const unsubscribe = firebaseAuth.onAuthStateChanged((firebaseUser: FirebaseUser | null) => {
        const localToken = tokenStorage.get();
        if (!firebaseUser && !localToken && !(session as any)?.apiToken) {
          setUser(null);
        }
      });
      return unsubscribe;
  ...
  const register = async (data: RegisterData) => {
    try {
      ...
      setAuthMethod('email');
      await syncWithNextAuth(response.token, response.user);
    } catch (err) {
