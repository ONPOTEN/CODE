...
export const CartProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Tải giỏ hàng từ localStorage khi mở ứng dụng
  useEffect(() => {
    ...
    if (savedCart) {
      try {
        ...
        console.error('Không thể tải giỏ hàng từ localStorage:', error);
      }
    }
    setIsHydrated(true);
  }, []);

  // Lưu giỏ hàng vào localStorage khi mặt hàng thay đổi
  useEffect(() => {
    if (isHydrated) {
  ...
  const addToCart = (item: Omit<CartItem, 'quantity'>) => {
    setItems((prevItems) => {
      // Kiểm tra xem mặt hàng với cùng postId, shopId và thuộc tính đã tồn tại chưa
      ...
      if (existingItem) {
        // Tăng số lượng nếu mặt hàng đã tồn tại
