...
export default function FirebasePhonePasswordSetupPage() {
  ...
  const handlePasswordSetup = async (e: FormEvent) => {
    ...
    try {
      ...
      // Step 4: Register with Laravel backend to create user record and associate phone
      console.log(`[Phone Password Setup] Registering with Laravel backend...`);

      const response = await apiRequest('/auth/firebase-register', {
        method: 'POST',
        body: JSON.stringify({
      ...
      if (response.success) {
        console.log(`[Phone Password Setup] User registered successfully`);

        // Step 5: Save to localStorage for future phone logins
        const phoneAuthData = {
          phoneNumber,
          password,
        ...
        // Step 6: Save the JWT token from Laravel
        if (response.token) {
