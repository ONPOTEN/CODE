...
/**
 * Get a presigned URL for video upload from the backend
 * @param filename - The name of the video file to upload
...
export const getVideoPresignedUrl = async (
  ...
  console.log('[s3-upload] Requesting video presigned URL for:', filename);

  const response = await fetch(`${API_BASE_URL}/s3/video/presigned-url`, {
    method: 'POST',
    headers: {
  ...
  console.log('[s3-upload] Video presigned URL response status:', response.status);

  if (!response.ok) {
    const errorText = await response.text();
    console.error('[s3-upload] Error response:', errorText);
    throw new Error(`Failed to get video presigned URL: ${response.statusText}`);
  }

  const data = await response.json();
  console.log('[s3-upload] Successfully got video presigned URL:', {
 ...
 * @returns Object containing the presigned URL and file key
 */
