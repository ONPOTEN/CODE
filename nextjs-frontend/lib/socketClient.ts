...
export const offGroupMessage = (callback?: (data: any) => void) => {
  const socket = getSocket();
  if (callback) {
    socket?.off('group-message', callback);
  } else {
...
export const offGroupUserJoined = (callback?: (data: any) => void) => {
  const socket = getSocket();
  if (callback) {
    socket?.off('group-user-joined', callback);
  } else {
...
export const offGroupUserLeft = (callback?: (data: any) => void) => {
  const socket = getSocket();
  if (callback) {
    socket?.off('group-user-left', callback);
  } else {
...
export const offGroupUserTyping = (callback?: (data: any) => void) => {
  const socket = getSocket();
  if (callback) {
    socket?.off('group-user-typing', callback);
  } else {
