...
class S3Controller extends Controller
{
     ...
     * @return JsonResponse
     */
    public function generateVideoPresignedUrl(Request $request): JsonResponse
    {
        try {
            \Log::info('[S3] Video presigned URL request received');
            ...
            $userId = $request->user()->ID ?? 'anonymous';
            $timestamp = time();
            $random = substr(md5($timestamp . $userId), 0, 10);
            $fileKey = "shop_videos/videos_temp/{$userId}/{$timestamp}_{$random}_{$filename}";

            \Log::info('[S3] Generated video file key', ['file_key' => $fileKey]);

            // Generate presigned URL (valid for 60 minutes for large videos)
            $cmd = $this->s3Client->getCommand('PutObject', [
                'Bucket' => $bucket,
                'Key' => $fileKey,
                'ContentType' => $contentType,
            ...
            $presignedRequest = $this->s3Client->createPresignedRequest($cmd, '+60 minutes');
            $presignedUrl = (string)$presignedRequest->getUri();

            \Log::info('[S3] Video presigned URL generated successfully', [
                'file_key' => $fileKey,
                'url_length' => strlen($presignedUrl),
            ]);
            ...
            return response()->json([
                'success' => false,
                'message' => 'An error occurred while generating video presigned URL',
                'error' => $e->getMessage(),
            ], 500);
        }
    }
