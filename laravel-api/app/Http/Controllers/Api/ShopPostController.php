...
class ShopPostController extends Controller
{
    ...
    public function store(Request $request, $shopId): JsonResponse
    {
        ...
        // Handle video upload - save to local temp and process in background
        $tempVideoPath = null;
        if ($request->hasFile('video')) {
            $videoFile = $request->file('video');

            try {
                \Log::info('Preparing video upload for background processing', [
                    'originalName' => $videoFile->getClientOriginalName(),
                    'size' => $videoFile->getSize(),
                    'mimeType' => $videoFile->getMimeType(),
                ]);

                // Save video directly to local disk root for background processing
                // No subdirectory — avoids silent failure if subdir doesn't exist
                Storage::disk('local')->makeDirectory('');
                $filename = time() . '_' . Str::random(10) . '.' . $videoFile->getClientOriginalExtension();
                $tempVideoPath = Storage::disk('local')->putFileAs('', $videoFile, $filename);

                if (!$tempVideoPath) {
                    throw new \Exception('Failed to save video to local temp storage.');
                }

                // Set initial status
                $createData['video_upload_status'] = 'pending';

                \Log::info('Video saved to temp storage, will be processed in background', [
                    'temp_path' => $tempVideoPath,
                ]);
            } catch (\Exception $e) {
                \Log::error('Failed to save video to temp storage', [
                    'error' => $e->getMessage(),
                ]);
            }
        }
        ...
        // Dispatch video upload job if video was uploaded
        if ($tempVideoPath) {
            \Log::info('Dispatching ProcessVideoUpload job', [
                'post_id' => $shopPost->id,
                'shop_id' => $shopId,
                'temp_path' => $tempVideoPath,
            ]);
            ProcessVideoUpload::dispatch($shopId, $shopPost->id, $tempVideoPath);
        }
    ...
    public function update(Request $request, $shopId, $id): JsonResponse
    {
        ...
        if ($request->hasFile('video')) {
            ...
            // Delete existing video from S3 if present
            if ($shopPost->video) {
                $fullS3Path = "shop_videos/{$shopPost->video}";
                if (\Storage::disk('s3')->exists($fullS3Path)) {
                    \Storage::disk('s3')->delete($fullS3Path);
                    \Log::info('Deleted existing video from S3', [
                        'post_id' => $shopPost->id,
                        'video_path' => $fullS3Path,
                    ]);
                }
            }

            try {
                \Log::info('Preparing video update for background processing', [
                    'post_id' => $shopPost->id,
                    'originalName' => $videoFile->getClientOriginalName(),
                    'size' => $videoFile->getSize(),
                ]);
                ...
                // Set initial status
                $shopPost->video_upload_status = 'pending';
                $shopPost->video_upload_error = null;

                \Log::info('Video saved to temp storage for update, will be processed in background', [
                    'post_id' => $shopPost->id,
                    'temp_path' => $tempVideoPath,
                ]);
            } catch (\Exception $e) {
                \Log::error('Failed to save video to temp storage for update', [
                    'post_id' => $shopPost->id,
                    'error' => $e->getMessage(),
                ]);
            }
        ...
        // Dispatch video upload job if video was uploaded
        if ($tempVideoPath) {
            \Log::info('Dispatching ProcessVideoUpload job for update', [
                'post_id' => $shopPost->id,
                'shop_id' => $shopId,
                'temp_path' => $tempVideoPath,
            ]);
    ...
    /**
     * Upload video to an existing post (background processing)
     */
    public function uploadVideo(Request $request, $shopId, $id): JsonResponse
    {
        $shopPost = ShopPost::where('shop_id', $shopId)->findOrFail($id);
        ...
        if ($shopPost->shop->user_id !== $request->user()->ID) {
            return response()->json([
                'message' => 'Unauthorized. You can only upload videos to your own shop posts.',
            ], 403);
        }
        ...
        try {
            \Log::info('Uploading video to existing post in background', [
                'post_id' => $shopPost->id,
                'shop_id' => $shopId,
                'originalName' => $videoFile->getClientOriginalName(),
                'size' => $videoFile->getSize(),
            ...
            \Log::info('Video saved to temp storage, dispatching job', [
                'post_id' => $shopPost->id,
                'shop_id' => $shopId,
                'temp_path' => $tempVideoPath,
            ]);

            // Dispatch video upload job
            ProcessVideoUpload::dispatch($shopId, $shopPost->id, $tempVideoPath);
     ...
     */
    public function getVideoStatus(Request $request, $shopId, $id): JsonResponse
    {
        $shopPost = ShopPost::where('shop_id', $shopId)->findOrFail($id);
        ...
        return response()->json([
            'post_id' => $shopPost->id,
            'video_upload_status' => $shopPost->video_upload_status,
            'video_upload_error' => $shopPost->video_upload_error,
            'video' => $shopPost->video,
        ]);
    }
