...
use App\Jobs\ProcessPostVideoUpload;
...
class PostController extends Controller
{
    ...
    /**
     * Upload video for a post (processes via queue)
     */
    public function uploadVideo(Request $request, $id): JsonResponse
    {
        $post = WpPost::findOrFail($id);
        $user = $request->user();
        ...
        try {
            ...
            if (!$tempVideoPath) {
                throw new \Exception('Failed to save temp video file to local storage');
            }
            ...
            // Dispatch job
            ProcessPostVideoUpload::dispatch($post->ID, $user->ID, $tempVideoPath);
     ...
     */
    public function getVideoStatus(Request $request, $id): JsonResponse
    {
        $post = WpPost::findOrFail($id);
        $user = $request->user();
        ...
        $statusMeta = $post->meta()->where('meta_key', '_video_upload_status')->first();
        $errorMeta = $post->meta()->where('meta_key', '_video_upload_error')->first();
        $videoMeta = $post->meta()->where('meta_key', '_post_video')->first();

        $videoUrl = null;
        if ($videoMeta) {
            try {
                $videoUrl = \Storage::disk('s3')->url($videoMeta->meta_value);
            } catch (\Exception $e) {
                $videoUrl = asset('storage/' . $videoMeta->meta_value);
            }
        }

        return response()->json([
            'post_id' => $post->ID,
            'video_upload_status' => $statusMeta ? $statusMeta->meta_value : null,
            'video_upload_error' => $errorMeta ? $errorMeta->meta_value : null,
            'video' => $videoUrl,
        ]);
    }
